...
dialect.select('t.f1 AS f1,t.f2 AS f2,t2.f3 AS f3').from_('t').join('t2',{'t.id':'t2.id'},'inner').where({'f1':'2'}).limit(100,100).make_view('my_view')
...
query_soft_view = dialect.select().from_('my_view').where({'f2':'3'}, 'OR').where({'f2':'1'}, 'OR').sql()
