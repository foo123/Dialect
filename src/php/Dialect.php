...
class GrammarTemplate
{
    ...
    public static function multisplit($tpl, $delims, $postop = false)
    {
        ...
        while ($i < $l)
        {
            ...
            elseif ($IDR === $delim)
            {
                ...
                elseif (!$optional && (null === $cur_arg->name))
                {
                    $cur_arg->name = $argument;
                    $cur_arg->key = $nested;
                    $cur_arg->stpl = $template;
    ...
    public static function optional_block($args, $block, &$SUB, &$FN, $index = null, $alignment = '', $orig_args = null)
    {
        ...
        elseif (!$arr && ($block->start === $block->end))
        {
...
class Dialect
{
    ...
    public function prepareTpl($tpl /*, $query = null, $left = null, $right = null*/)
    {
        if (!empty($tpl))
        {
            ...
            $pattern = '/' . $left . '(([rlfids]:)?[0-9a-zA-Z_]+)' . $right . '/';
                ...
                switch ($type)
                {
                    ...
                    case 'd':
                        if (is_array($v))
                        {
                            ...
                            $params[$k] = implode(',', $this->floatval((array)$v));
                        }
                        else
                        {
                            // float
                            $params[$k] = $this->floatval($v);
                        }
                        break;
                ...
                if (isset($args[$param]))
                {
                    ...
                    switch ($type)
                    {
                        ...
                        case 'd':
                            ...
                            {
                                // array of floats param
                                $param = implode(',', $this->floatval((array)$args[$param]));
                            }
                            else
                            {
                                // float
                                $param = $this->floatval($args[$param]);
                            }
                            break;
    ...
    public function Transaction($options, $transact_clause = 'transact')
    {
        ...
        if (!empty($options['statements']))
        {
            ...
            $this->clus['statements'] = empty($this->clus['statements']) ? $statements : array_merge($this->clus['statements'], $statements);
        }
        return $this;
    }
    ...
    public function Drop($tables = '*', $options = null, $drop_clause = 'drop')
    {
        ...
        $this->clus['temporary'] = !empty($options['temporary']) ? 1 : null;
        $this->clus['drop_tables'] = empty($this->clus['drop_tables']) ? $tables : array_merge($this->clus['drop_tables'], $tables);
        return $this;
    }
    ...
    public function Select($columns = '*', $select_clause = 'select')
    {
        ...
        $this->clus['select_columns'] = empty($this->clus['select_columns']) ? $columns : array_merge($this->clus['select_columns'], $columns);
        return $this;
    }
    ...
    public function Union($selects, $all = false, $union_clause = 'union')
    {
        ...
        $this->clus['union_selects'] = empty($this->clus['union_selects']) ? (array)$selects : array_merge($this->clus['union_selects'], (array)$selects);
    ...
    public function Insert($tables, $columns, $insert_clause = 'insert')
    {
        ...
        else
        {
            ...
            $this->clus['insert_tables'] = empty($this->clus['insert_tables']) ? $tables : array_merge($this->clus['insert_tables'], $tables);
            $this->clus['insert_columns'] = empty($this->clus['insert_columns']) ? $columns : array_merge($this->clus['insert_columns'], $columns);
        }
        return $this;
    }
            ...
            if (!empty($values[$i]))
            {
                ...
                foreach ((array)$values[$i] as $val)
                {
                    if (is_array($val))
                    {
                        ...
                        elseif (isset($val['int']))
                        {
                            $vals[] = $this->intval($val['int']);
                        }
                        elseif (isset($val['float']))
                        {
                            $vals[] = $this->floatval($val['float']);
                        }
                    ...
                    {
                        $vals[] = null === $val ? 'NULL' : (is_float($val) || is_int($val) ? (string)$val : $this->quote($val));
                    }
                }
    ...
    public function Update($tables, $update_clause = 'update')
    {
        ...
        else
        {
            if (is_string($tables)) $tables = explode(',', $tables);
            $tables = $this->refs($tables, $this->tbls);
            $this->clus['update_tables'] = empty($this->clus['update_tables']) ? $tables : array_merge($this->clus['update_tables'], $tables);
        }
        return $this;
    }
    ...
    public function Set($fields_values)
    {
        ...
        foreach ($fields_values as $f => $value)
        {
            ...
            if (is_array($value))
            {
                ...
                {
                    $set_values[] = "$field = " . $this->intval($value['int']);
                }
                elseif (isset($value['float']))
                {
                    $set_values[] = "$field = " . $this->floatval($value['float']);
                }
                ...
                {
                    $set_values[] = "$field = $field + " . $this->numval($value['increment']);
                }
                elseif (isset($value['decrement']))
                {
                    $set_values[] = "$field = $field - " . $this->numval($value['decrement']);
                }
            ...
            {
                $set_values[] = "$field = " . (null === $value ? 'NULL' : (is_float($value) || is_int($value) ? (string)$value : $this->quote($value)));
            }
        }
    ...
    public function From($tables)
    {
        ...
        else
        {
            if (is_string($tables)) $tables = explode(',', $tables);
            $tables = $this->refs($tables, $this->tbls);
            $this->clus['from_tables'] = empty($this->clus['from_tables']) ? $tables : array_merge($this->clus['from_tables'], $tables);
        }
        return $this;
    }
    ...
    public function Where($conditions, $boolean_connective = "AND")
    {
        ...
        $boolean_connective = strtoupper((string)$boolean_connective);
    ...
    public function conditions($conditions, $can_use_alias = false)
    {
        ...
        foreach ($conditions as $f => $value)
        {
            if (is_array($value))
            {
                ...
                {
                    $conds[] = "$field LIKE " . ('raw' === $type ? (string)$value['like'] : $this->like($value['like']));
                }
                elseif (isset($value['not_like']))
                {
                    $conds[] = "$field NOT LIKE " . ('raw' === $type ? (string)$value['not_like'] : $this->like($value['not_like']));
                }
                ...
                elseif (isset($value['in']))
                {
                    ...
                    elseif (('int' === $type || 'integer' === $type) || is_int($v[0]))
                    {
                        $v = $this->intval($v);
                    }
                    elseif ('float' === $type || is_float($v[0]))
                    {
                        $v = $this->floatval($v);
                    }
                    else
                ...
                elseif (isset($value['between']))
                {
                    ...
                    if (!isset($v[0]) || (null === $v[0]))
                    {
                        ...
                        elseif (('int' === $type || 'integer' === $type) || is_int($v[1]))
                        {
                            $v[1] = $this->intval($v[1]);
                        }
                        elseif ('float' === $type || is_float($v[1]))
                        {
                            $v[1] = $this->floatval($v[1]);
                        }
                        else
                        {
                            $v[1] = $this->quote($v[1]);
                    ...
                    else
                    {
                        ...
                        elseif (('int' === $type || 'integer' === $type) || (is_int($v[0]) && is_int($v[1])))
                        {
                            $v = $this->intval($v);
                        }
                        elseif ('float' === $type || (is_float($v[0]) && is_float($v[1])))
                        {
                            $v = $this->floatval($v);
                        }
                        else
                ...
                elseif (isset($value['gt']) || isset($value['gte']))
                {
                    ...
                    elseif (('int' === $type || 'integer' === $type) || is_int($v))
                    {
                        $v = $this->intval($v);
                    }
                    elseif ('float' === $type || is_float($v))
                    {
                        $v = $this->floatval($v);
                    }
            ...
            else
            {
                $field = $this->refs($f, $this->cols);
                $field = $field[0]->{$fmt};
                $conds[] = null === $value ? "$field IS NULL" : ("$field = " . (is_float($value) || is_int($value) ? (string)$value : $this->quote($value)));
            }
        }
    ...
    public function intval($v)
    {
        ...
        return is_int($v) ? $v : intval((string)$v, 10);
    }

    public function floatval($v)
    {
        if (is_array($v))
        {
            foreach ($v as $i => $vi) $v[$i] = $this->floatval($vi);
            return $v;
        }
        return is_float($v) ? $v : floatval((string)$v, 10);
    }

    public function numval($v)
    {
        if (is_array($v))
        {
            foreach ($v as $i => $vi) $v[$i] = $this->numval($vi);
            return $v;
        }
        return is_float($v) || is_int($v) ? $v : floatval((string)$v, 10);
    }
    ...
    public function multi_like($f, $v, $trimmed = true)
    {
        ...
        $like = "$f LIKE ";
        $ORs = explode(',', $v);
        if ($trimmed) $ORs = array_values(array_filter(array_map('trim', $ORs), 'strlen'));
        foreach ($ORs as &$OR)
        {
            $ANDs = explode('+', $OR);
            if ($trimmed) $ANDs = array_values(array_filter(array_map('trim', $ANDs), 'strlen'));
